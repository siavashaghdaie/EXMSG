...
interface ChatState {
  ...
  setReplyingTo: (
    data: { messageId: string; content: string; senderName: string } | null
  ) => void;
...
export const useChatStore = create<ChatState>((set, get) => ({
  ...
  fetchMessages: async (conversationId: string, cursor?: string) => {
    ...
    try {
      const response: PaginatedMessages = await api.getMessages(
        conversationId,
        cursor,
        50
      );
  ...
  handleReactionAdded: (reaction: ReactionEvent) => {
    set((state) => {
      const messages = new Map(state.messages);
      const conversationMessages =
        messages.get(reaction.conversationId) || [];
      const updated = conversationMessages.map((msg) => {
  ...
  handleTypingStart: (typing: TypingEvent) => {
    set((state) => {
      ...
      const conversationIndicators =
        indicators.get(typing.conversationId) || [];
      const filtered = conversationIndicators.filter(
  ...
  handleTypingStop: (typing: TypingEvent) => {
    set((state) => {
      ...
      const conversationIndicators =
        indicators.get(typing.conversationId) || [];
      const updated = conversationIndicators.filter(
        (t) => t.userId !== typing.userId
      );
      indicators.set(typing.conversationId, updated);
      return { typingIndicators: indicators };
